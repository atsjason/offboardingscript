...
#Updated CheckProgramInstalled Method: 1/18/2022
Function CheckProgramInstalled($programName) {
     $installed = (Get-WmiObject -Class Win32_Product -Filter "Name LIKE '$programName'").name
     if (-not [System.String]::IsNullOrEmpty($installed))
     {
         return $true
     }
...
)
{
    ...
    If (CheckProgramInstalled('%Microsoft Visual C++ 2013 x64%') -and CheckProgramInstalled('%Microsoft Visual C++ 2013 x64%') -and AgentIsOnFileSystem)
    {
